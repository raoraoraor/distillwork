...
import wandb
from tqdm import tqdm   # 注意环境中要新安装tqdm包
...
def train_one_epoch(net, MODEL, criterion, optimizer, bnm_scheduler, TRAIN_DATALOADER,
    ...
    return {
        'total_loss': stat_dict.get('total_loss', 0.0)/len(TRAIN_DATALOADER),
        'original_loss': stat_dict.get('original_loss', 0.0)/len(TRAIN_DATALOADER),
        'distill_loss': stat_dict.get('distill_loss', 0.0)/len(TRAIN_DATALOADER),
        'feat_distill_loss': stat_dict.get('feat_distill_loss', 0.0)/len(TRAIN_DATALOADER),
        'pseudo_label_loss': stat_dict.get('pseudo_label_loss', 0.0)/len(TRAIN_DATALOADER)
    }
...
def train_or_evaluate(start_epoch, net, MODEL, net_no_ddp, criterion, optimizer, bnm_scheduler, train_sampler,
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        if is_primary() and FLAGS.if_wandb:
            wandb.log({
                "train/total_loss": stat_dict_loss['total_loss'],
                "train/original_loss": stat_dict_loss['original_loss'],
                "train/distill_loss": stat_dict_loss.get('distill_loss', 0),
                "train/feat_distill_loss": stat_dict_loss.get('feat_distill_loss', 0),
                "train/pseudo_label_loss": stat_dict_loss.get('pseudo_label_loss', 0)
            }, step=epoch)
