#除去文件名外其余内容同wrong中的.sh选项 教师imvotenet 学生 votenet  刚刚能跑通distill_loss（注意此处有更改） 其余损失函数还未调试
import os
import sys
import numpy as np
...
def load_teacher_model(MODEL,TRAIN_DATASET,FLAGS, DATASET_CONFIG, num_input_channel):
    ...
    teacher_MODEL = importlib.import_module('imvotenet') #把MODEL的事情忽略掉先
    teacher_model = teacher_MODEL.ImVoteNet( #已修改MODEL.路径问题
...
def train_one_epoch(net,MODEL,criterion,optimizer,bnm_scheduler,TRAIN_DATALOADER,teacher_model=None,teacher_MODEL=None,teacher_TRAIN_DATALOADER=None):
    ...
    for batch_idx, batch_data_label in enumerate(teacher_TRAIN_DATALOADER):
        ...
        teacher_inputs = {'point_clouds': batch_data_label['point_clouds']}
        teacher_inputs.update({'scale': batch_data_label['scale'],
        ...
        with torch.no_grad():
            teacher_end_points = teacher_model(teacher_inputs)
        ...
        for key in student_end_points:
            print(key)
        print("孙笑川258")
        for key in teacher_end_points:
            print(key)
            
        distill_loss = F.kl_div(
            F.log_softmax(student_end_points['pc_only_sem_cls_scores'], dim=-1),
...
def train_or_evaluate(start_epoch,net,MODEL,net_no_ddp,criterion,optimizer,bnm_scheduler,train_sampler,TRAIN_DATALOADER,TEST_DATALOADER, teacher_model=None,teacher_MODEL=None,teacher_TRAIN_DATALOADER=None):
    global EPOCH_CNT 
    loss = 0
    ...
    for epoch in range(start_epoch, MAX_EPOCH):
        ...
        stat_dict_loss = train_one_epoch(net,MODEL,criterion,optimizer,bnm_scheduler,TRAIN_DATALOADER,teacher_model,teacher_MODEL,teacher_TRAIN_DATALOADER)
...
def main_dist(local_rank, FLAGS):
    ...
    teacher_TRAIN_DATASET = DetectionVotesDataset('train', 
                                            num_points=NUM_POINT,
                                            augment=True,
                                            use_color=FLAGS.use_color, 
                                            use_height=(not FLAGS.no_height),
                                            use_imvote=True, #教师模型使用imvotenet
                                            max_imvote_per_pixel=FLAGS.max_imvote_per_pixel,
                                           )
    ...
    # ===== 新增代码开始 =====
    # 加载教师模型（仅主进程）
    teacher_model,teacher_MODEL = load_teacher_model(MODEL,TRAIN_DATASET,FLAGS, DATASET_CONFIG, num_input_channel) if is_primary() else None
    ...
    teacher_TRAIN_DATALOADER = DataLoader(teacher_TRAIN_DATASET, batch_size=BATCH_SIZE,
        sampler=train_sampler, num_workers=FLAGS.num_workers, worker_init_fn=my_worker_init_fn)
